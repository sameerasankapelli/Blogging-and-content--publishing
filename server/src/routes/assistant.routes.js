...
async function geminiReply(message) {
  const { text, used, reason } = await generateText({
    system: 'You are a helpful assistant for a university blogging app (Vignan University). Be concise, list steps when helpful, and avoid unsafe content.',
    prompt: message,
  });
  return { text, used, reason };
}
...
router.post('/', auth(false), async (req, res) => {
  ...
  // Use Gemini; else local tips
  let llm = await geminiReply(message);
  ...
  res.json({ reply: responseText, suggestions: [...posts, ...extraChips], source: llm.used ? 'gemini' : 'fallback', reason: debug ? llm.reason : undefined });
});
