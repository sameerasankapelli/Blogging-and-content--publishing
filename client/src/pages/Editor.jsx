...
export default function Editor() {
  ...
  function scheduleSave() {
    ...
    saveTimer.current = setTimeout(async () => {
      setSaving(true)
      try {
        const payload = { title, markdown, tags: tagsToArray(tags), coverUrl: coverUrl || undefined, images: attachments }
        if (!draftId) {
...
function AIToolbar({ markdown, title, setMarkdown, setTags }) {
  ...
  async function onSummarize() {
    ...
    if (data?.summary) setMarkdown(prev => `> Summary\n> ${data.summary.replace(/\n/g, '\n> ')}\n\n` + prev)
  }
  ...
  async function onTweetThread() {
    const data = await call('/ai/tweet_thread', { text: `${title}\n\n${markdown}` })
    if (data?.tweets?.length) {
      const block = data.tweets.map((t, i) => `- ${t}`).join('\n')
  ...
  return (
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
      ...
      <button className="btn btn-ghost btn-small" onClick={onTweetThread} disabled={busy}>Tweet thread</button>
    </div>
  )
}
