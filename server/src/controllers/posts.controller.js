...
const createDraft = [
  ...
  async (req, res) => {
    ...
    const post = await Post.create({ title, slug, markdown, tags, coverUrl, images, author: req.user.id, status: 'draft' });
    res.json(post);
  },
];
...
const publish = async (req, res) => {
  ...
  if (req.body.coverUrl) post.coverUrl = req.body.coverUrl;
  await post.save();
  res.json(post);
};
